 ...
 * @param {object} [raw] 组合行传入的原始 config
 * @returns {{ maxAttempts: number, candidates: Array<{provider: string, model: string}>, autoFill: boolean, system: string, modes: object }} 规范化后的配置
 */
...
/**
 * 采集项目上下文（目录树 + 依赖/配置/入口文件），带预算上限与路径防护。
 * @param {object} fs DSH 文件系统服务（resolve/stat/listDir/readText）
 * @param {string} cwd 会话工作目录
 * @param {object} cfg 模式配置（depth/treeLines/treeChars/fileChars/contentChars）
 * @returns {Promise<string|null>} 拼接好的上下文文本；目录不可读时返回 null
 */
async function collectContext(fs, cwd, cfg) {
  // DSH 的 fs.resolve() 返回 { targetKey, displayPath } 目标对象（targetKey 为
  // realpath 归一化后的稳定标识）；旧测试桩可能直接返回字符串。统一取路径字符串，
  // 防止把对象当字符串用（workspaceRoot.replace 抛 TypeError → 上下文采集失败）。
  const targetPath = (t) => (typeof t === 'string' ? t : (t && t.targetKey) || '')
  // 深度防御：确保 cwd 不会逃逸出工作区根目录
  const workspaceRoot = await fs.resolve('.')
  const root = await fs.resolve(cwd)
  const rootPath = targetPath(root)
  // 归一化后比较前缀，防止路径遍历攻击（含分隔符边界：/workspace 不能匹配 /workspace-else）
  const base = targetPath(workspaceRoot).replace(/[\\/]+$/, '')
  const inside = rootPath === base || rootPath.startsWith(base + '/') || rootPath.startsWith(base + '\\')
  if (!inside) {
    throw new Error('Access denied: cwd outside workspace root')
  }
 ...
 * @param {object} mode 模式配置（含 extra 说明）
 * @param {object} cfg 全局配置（system 覆盖项）
 ...
 * @param {object} mode 模式配置（needsContext）
...
function buildUserMessage(mode, context, draft, answers, baseEnhanced) {
  ...
  if (answers.length > 0) {
    ...
    return contextPart + '用户确认的信息：\n' + lines.join('\n') + '\n\n原始提示词：\n' + draft +
      '\n\n请基于以上项目上下文与用户确认的信息重写并增强上面的原始提示词，直接输出增强结果。'
  }
 ...
 * @param {object} llm DSH 模型服务（listProviders/listModels）
 * @param {object} [defaultModel] 会话默认模型（currentSelection()）
 * @param {object} cfg 全局配置（candidates/autoFill/maxAttempts）
 ...
 * @param {object} llm DSH 模型服务（stream）
 * @param {{provider: string, model: string}} candidate 候选模型
 * @param {string} system system prompt
 * @param {string} user 用户消息（增强请求）
 * @param {object} cfg 模式配置（temperature/maxTokens）
 ...
 * @param {object} llm DSH 模型服务
 * @param {{provider: string, model: string}} candidate 候选模型
 * @param {string} system system prompt
 * @param {string} user 用户消息
 * @param {object} mode 模式配置（effort/temperature/maxTokens）
 * @param {AbortSignal} [signal] 取消信号
 ...
 * @param {object} req Node HTTP 请求对象
 ...
 * @param {object} ctx Cordis 注入上下文（webServer/sessions/fs/llm/agentDefaultModel）
 * @param {object} [config] 组合行 config（见 resolveConfig）
 */
      ...
      handler: async (req, res) => {
        try {
          ...
          if (mode.needsContext) {
            ...
            const fs = ctx.get('fs')
            if (!fs) {
              markRequest('failed', { msg: 'fs 服务不可用' })
              return sendJson(res, { ok: false, message: '文件系统服务不可用，无法读取项目上下文。' })
            }
            if (!cwd) {
              markRequest('failed', { msg: '会话 header 缺少 cwd' })
              return sendJson(res, { ok: false, message: '当前会话未关联工作目录，无法读取项目上下文。' })
            }

            try { context = await collectContext(fs, cwd, mode) } catch (e) { context = '' }
            if (!context || !context.trim()) {
              markRequest('failed', { msg: 'collectContext 未产出上下文' })
          ...
          if (canceled) {
            markRequest('canceled', { revise })
            return sendJson(res, { ok: false, canceled: true, message: '已取消增强。' })
          }
          if (!enhanced) {
            markRequest('failed', { revise, msg: failures.join('；') })
          ...
          markRequest('ok', { revise, len: enhanced.length, conf: confirmations.length, model: usedModel })
