...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * @param {object} ctx Cordis 注入上下文（slots/timer）
     */
    function apply(ctx) {
      ctx.effect(() => {
      ...
      const runRevise = async (props, mode, answers) => {
        ...
        try {
          ...
          if (result && result.ok && typeof result.enhanced === 'string' && result.enhanced) {
            ...
            publish({ phase: 'notice', kind: 'error', text: (result && result.message) || '修订失败，已保留上一版增强结果。' })
          }
        } catch (e) {
          if (ctrl.signal.aborted) {
            publish({ phase: 'notice', kind: 'info', text: '已取消修订。' })
            ...
            publish({ phase: 'notice', kind: 'error', text: '修订失败：' + (e && e.message ? e.message : String(e)) })
          }
        } finally {
          if (activeAbort === ctrl) activeAbort = null
        }
